...
export default function FdfInputsPage() {
  ...
  // ✅ apply preset in ONE setState (no cascading)
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...prev,
      ...preset,
      department: prev.department, // ensure department doesn't get overridden
    }));
  }, [form.department]);
        ...
        <div className="fdf-panel">
          ...
          <form className="fdf-form" onSubmit={onSubmit}>
            <section>
              ...
              <div className="fdf-noteBox" style={{ background: "transparent", border: "none", padding: 0 }}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm((prev) => ({ ...prev, department: e.target.value as Department }))
                  }
                >
                  {DEPARTMENTS.map((d) => (
            ...
            <section>
              ...
              <MoneyRow label="Quarter Revenue" value={form.revenue} onChange={(v) => setNumber("revenue", v)} />
              <MoneyRow label="Quarter COGS" value={form.cogs} onChange={(v) => setNumber("cogs", v)} />
              <MoneyRow
              ...
              <div className="fdf-noteBox" style={{ marginTop: 10 }}>
                <div className="fdf-inlineStats">
                  <span>Gross Profit: <b>${money(preview.gp)}</b></span>
                  <span>Operating Profit: <b>${money(preview.op)}</b></span>
                  <span>Margin: <b>{Math.round(preview.margin * 1000) / 10}%</b></span>
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <NumberRow label="DSO (Days Sales Outstanding)" value={form.dso} onChange={(v) => setNumber("dso", v)} suffix="days" />
              <NumberRow label="DIO (Days Inventory Outstanding)" value={form.dio} onChange={(v) => setNumber("dio", v)} suffix="days" />
              <NumberRow label="DPO (Days Payables Outstanding)" value={form.dpo} onChange={(v) => setNumber("dpo", v)} suffix="days" />
            </section>

            <section>
              <h2 className="fdf-sectionTitle">Additional Inputs</h2>
              <NumberRow label="Net Promoter Score (NPS)" value={form.nps} onChange={(v) => setNumber("nps", v)} />
              <PercentRow
            ...
            <div style={{ marginTop: 6 }}>
              <button type="submit" className="fdf-submit">Submit</button>
              <div className="fdf-footerHint">Department presets apply automatically (single state update).</div>
            </div>
          </form>
        </div>
      </div>
  ...
  return (
    <div className="fdf-row" style={{ marginTop: 10 }}>
      ...
      <div className="fdf-control">
        ...
        <input className="fdf-input" type="number" value={value} onChange={(e) => onChange(Number(e.target.value))} />
      </div>
    </div>
  );
}
